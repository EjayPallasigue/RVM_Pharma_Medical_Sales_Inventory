...
class Insert extends CI_Controller
{
	...
	public function create_staff() {
        if ($this->session->userdata('username') != '') { // Check Login
            ...
            if ($this->form_validation->run() == FALSE) {
                redirect('ShowForm/manage_staff/empty', 'refresh'); // If form not validated
            } else {
                $username = $this->input->post('username'); // Get data from file to variable

                if ($this->CommonModel->is_username_exists($username)) {
                    redirect('ShowForm/manage_staff/USERNAME_ALREADY_TAKEN', 'refresh');
                } else {
